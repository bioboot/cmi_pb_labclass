...
2022-03-07  (14:51:06 on Mon, Mar 07)  



# Background  

Pertussis (more commonly known as **whooping cough**) is a highly contagious respiratory disease caused by the bacterium *Bordetella pertussis*. People of all ages can be infected leading to violent coughing fits followed by a high-pitched intake of breath that sounds like "whoop". Infants and toddlers have the highest risk for severe complications and death. Recent estimates from the WHO suggest that approximately 16 million cases and 200,000 infant deaths are due to pertussis annually [1](https://pubmed.ncbi.nlm.nih.gov/20466419/).
...
- CMI-PB project is an attempt to make data on this question open an examinable by all.
...
- **Q15.** Filter to pull out only two specific antigens for analysis and create a boxplot for each. You can chose any you like. Below I picked a "control" antigen (**"Measles"**, that is not in our vaccines) and a clear antigen of interest (**"FIM2/3"**, extracellular fimbriae proteins from *B. pertussis* that participate in substrate attachment). 
...
# 5. Working with CMI-PB RNASeq data

For RNA-Seq data the API query mechanism quickly hits the web browser interface limit for file size. We will present alternative download mechanisms for larger CMI-PB datasets in a later section. However, we can still do "targeted" RNA-Seq querys via the web accessible API. 
...
(answer: cells make antibodies, which are long lived)


Going further with `infancy_vac` status:
...
> OPTIONAL Q. Facet by biological sex `+ facet_wrap(vars(biological_sex))` any obvious difference?





# 6. [OPTIONAL] Obtaining larger RNA-Seq datasets
...
Once you have done this we can begin to investigate the relationship between mRNA levels on different days and ultimately address the next pressing questions: 

> **Key-questions:** 
>
> - Is RNA-Seq expression levels predictive of Ab titers?
> - What differentiates aP vs. wP primed individuals? 
> - What about decades after their first immunization? Do you know? Contact Bjoern for your trip to Sweden

 
There are immune responses not captured in antibody titers. We are trying to capture the ‘systems level’ response to antigen encounters (here: vaccination). Something must be different in aP vs. wP primed individuals. That is why CMI-PB make all their data available. 
